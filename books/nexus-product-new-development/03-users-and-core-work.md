...
![解決したかった課題を補助する白黒線画](/images/nexus-product-new-development/section/section-03-problem-fragmentation.png)
...
![主な利用者を補助する白黒線画](/images/nexus-product-new-development/section/section-03-users.png)
...
![最重要の業務フローを補助する白黒線画](/images/nexus-product-new-development/section/section-03-core-workflow.png)
...
![最初から決めなかったことを補助する白黒線画](/images/nexus-product-new-development/section/section-03-not-decided.png)


初期構想では将来的なSaaS化も候補にありました。しかし、最初の対象は自社利用です。

マルチテナント、高度な予測分析、会計連携、外部カレンダー連携は初期スコープから外しました。また、既存スプレッドシートのデータ移行も、ゼロから運用を開始する判断によって対象外になりました。

新規プロダクトでは、 **何を作れるか** よりも **最初にどの業務判断を改善するか** を絞ることが重要です。

RADARは、PM・プロジェクトリーダーと営業が、仮アサインから受注確定・正式化までの判断を共有するためのプロダクトです。
次章では、その判断を支えるコード、設計履歴、MVPの保存方法を見ます。
