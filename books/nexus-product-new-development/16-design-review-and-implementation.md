...
| 用語 | 説明 |
| :--- | :--- |
| `SYN` | 設計レビューで指摘された重大課題の管理IDプレフィックス。`SYN-001`のように連番で追跡する |
| RPO | Recovery Point Objective（目標復旧時点）。障害発生時に許容できる最大のデータ損失時間 |
| RTO | Recovery Time Objective（目標復旧時間）。障害発生からシステム復旧までに許容される最大時間 |
